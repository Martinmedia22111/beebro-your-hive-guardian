...
export default function GiftPage() {
  ...
  const { families, setGiftMode, setCartFamily } = useApp();
  const available = families.filter(f => f.status !== 'taken').slice(0, 3);

  const handleGiftFamily = (familyId: string) => {
      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="relative z-10 container max-w-2xl">
          ...
          <Button size="lg" onClick={() => navigate('/catalog')}>
            Выбрать семью для подарка <ChevronRight className="w-5 h-5 ml-1" />
          </Button>
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="container">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {[
              { icon: <Gift className="w-8 h-8" />, title: 'Подарить семью', desc: 'Выберите конкретную пчелиную семью и оформите опеку на имя получателя.', action: () => navigate('/catalog') },
              { icon: <Star className="w-8 h-8" />, title: 'Подарочный сертификат', desc: 'Сертификат на опеку без привязки к семье. Получатель выберет сам.', action: () => { setGiftMode(true); navigate('/catalog'); } },
              { icon: <Heart className="w-8 h-8" />, title: 'Премиум-подарок', desc: 'Расширенный формат: семья + мёд + сертификат + персональное поздравление.', action: () => navigate('/catalog') },
            ].map((item, i) => (
              <div key={i} className="bg-card rounded-xl border border-border p-8 text-center shadow-soft hover:shadow-card transition-all">
                ...
                <Button variant="outline" onClick={item.action}>Выбрать</Button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container max-w-2xl text-center">
          ...
          <div className="space-y-6 text-left">
            {[
              ...
              { n: '2', title: 'Заполните данные получателя', desc: 'Имя, поздравление, дата вручения.' },
              { n: '3', title: 'Оплатите', desc: 'Получите подарочный сертификат и ссылку.' },
              { n: '4', title: 'Подарите', desc: 'Отправьте по email или вручите лично.' },
            ].map((s, i) => (
